...
impl<P: Pipe> PythonParserBuilder<P> {
    ...
    pub fn load_class<'p>(py: Python<'p>,
                          module: &PyModule,
                          class_name: &str)
                          -> PyResult<PyObject> {
    ...
    pub fn create_options_dict<'p>(py: Python<'p>,
                                   init_options: &[(String, String)])
                                   -> PyResult<PyDict> {
    ...
    pub fn initialize_instance<'p>(py: Python<'p>,
                                   instance: &PyObject,
                                   options: PyDict)
                                   -> PyResult<()> {
    ...
    pub fn initialize_class<'p>(py: Python<'p>,
                                class: &PyObject,
...
fn python_register_callbacks(py: Python, dict: &mut PyDict) -> PyResult<()> {
    try!(python_register_callback(py,
                                  dict,
                                  "error",
                                  py_fn!(python_error_callback(error_message: &str))));
    try!(python_register_callback(py,
                                  dict,
                                  "info",
                                  py_fn!(python_info_callback(info_message: &str))));
    try!(python_register_callback(py,
                                  dict,
                                  "trace",
                                  py_fn!(python_trace_callback(trace_message: &str))));
    try!(python_register_callback(py,
                                  dict,
                                  "warning",
                                  py_fn!(python_warning_callback(warning_message: &str))));
    try!(python_register_callback(py,
                                  dict,
                                  "debug",
                                  py_fn!(python_debug_callback(debug_message: &str))));
    Ok(())
}

fn python_register_callback<F: ToPyObject>(py: Python,
                                           dict: &mut PyDict,
                                           name: &str,
                                           function: F)
                                           -> PyResult<()> {
...
impl<P: Pipe> ParserBuilder<P> for PythonParserBuilder<P> {
    ...
    fn build(self) -> Result<Self::Parser, Error> {
        ...
        match PythonParserBuilder::<P>::load_and_init_class(py,
                                                            &module_name,
                                                            &class_name,
                                                            &self.options) {
            Ok(parser_instance) => {
...
impl<P: Pipe> PythonParser<P> {
    pub fn process_parsing<'p>(&mut self,
                               py: Python<'p>,
                               logmsg: PyLogMessage,
                               message: &str)
                               -> PyResult<PyObject> {
